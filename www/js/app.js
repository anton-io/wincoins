...
class WinCoinsApp {
    ...
    try {
      ...
      if (!contractInitialized) {
        this.showNotification(
          "Failed to initialize smart contract. Please check your wallet connection.",
          "error"
        );
        return;
      }
      ...
      if (error.message.includes("Ethers.js")) {
        this.showNotification(
          "Ethers.js library failed to load. Please check your internet connection and refresh.",
          "error"
        );
      } else {
  ...
  setupEventListeners() {
    ...
    document
      ...
      .addEventListener("click", () => this.handleWalletDetailsClick());

    // Network selector
    document
      .getElementById("networkButton")
      .addEventListener("click", () => this.toggleNetworkDropdown());
    ...
    document
      ...
      .addEventListener("submit", (e) => this.handleCreateEvent(e));

    // Refresh events.
    document
      .getElementById("refreshEvents")
      .addEventListener("click", () => this.loadEvents());
  ...
  populateNetworkOptions() {
    ...
    mainnetContainer.innerHTML = mainnets
      .map(
        (network) => `
    ...
    testnetContainer.innerHTML = testnets
      .map(
        (network) => `
            <div class="network-option" data-network="${network.name}">
                <div class="network-info">
        ...
        document.querySelectorAll(".network-option").forEach((option) => {
          ...
          if (
            ...
            Object.keys(NETWORKS).find(
              (key) => NETWORKS[key].chainId === currentNetwork.chainId
            )
          ) {
  ...
  async switchToNetwork(networkName) {
    try {
      this.showNotification(
        `Switching to ${NETWORKS[networkName]?.chainName || networkName}...`,
        "info"
      );
      ...
      if (success) {
        ...
        this.showNotification(
          `Successfully switched to ${
            NETWORKS[networkName]?.chainName || networkName
          }`,
          "success"
        );
        this.closeNetworkDropdown();
  ...
  async reconnectToNetwork() {
    try {
      ...
      this.showNotification(
        "Please reconnect your wallet for the new network.",
        "warning"
      );
    }
  }
  ...
  setupOracleSelector() {
    const oracleSelect = document.getElementById("eventOracle");
    const customOracleGroup = document.getElementById("customOracleGroup");

    if (oracleSelect) {
      oracleSelect.addEventListener("change", (e) => {
        if (e.target.value === "custom") {
          customOracleGroup.style.display = "block";
          document.getElementById("customOracleAddress").required = true;
        } else {
          customOracleGroup.style.display = "none";
          document.getElementById("customOracleAddress").required = false;
        }
      });
    }
  }
      ...
      if (window.ethereum) {
        ...
        if (accounts.length > 0) {
          ...
          document.getElementById(
            "walletAddress"
          ).textContent = `${address.slice(0, 6)}...${address.slice(-4)}`;
  ...
  async connectWallet() {
    try {
      ...
      document.getElementById("connectWallet").style.display = "none";
      document.getElementById("walletStatus").classList.remove("hidden");
      document.getElementById("walletAddress").textContent = `${address.slice(
        0,
        6
      )}...${address.slice(-4)}`;
  ...
  disconnectWallet() {
    ...
    document.getElementById("eventsContainer").innerHTML =
      '<div class="loading">Please connect your wallet to view events.</div>';
    document.getElementById("userPredictionsContainer").innerHTML =
      "<p>Please connect your wallet to view your predictions.</p>";
  ...
  async updateBalance() {
    if (this.isConnected) {
      try {
        ...
        document.getElementById("walletBalance").textContent = `${new Decimal(
          balance
        ).toFixed(4)} ${symbol}`;
      } catch (error) {
  ...
  async handleCreateEvent(e) {
    ...
    try {
      ...
      const oracleSelection = document.getElementById("eventOracle").value;
      ...
      if (predictionDuration < 1) {
        this.showNotification(
          "Prediction deadline must be at least 1 minute from now.",
          "error"
        );
        return;
      }
      ...
      if (!oracleSelection) {
        this.showNotification(
          "Please select how this event will be resolved.",
          "error"
        );
        return;
      }
      ...
      if (oracleSelection === "self") {
      ...
      } else if (oracleSelection === "custom") {
        const customAddress = document
          .getElementById("customOracleAddress")
          .value.trim();
        if (!customAddress) {
          this.showNotification(
            "Please enter a custom oracle address.",
            "error"
          );
          return;
        }
        if (!ethers.utils.isAddress(customAddress)) {
          this.showNotification(
            "Please enter a valid Ethereum address for the oracle.",
            "error"
          );
          return;
        }
        oracleAddress = customAddress;
      }

      this.showNotification(
        "Creating event... Please confirm the transaction.",
        "info"
      );

      const eventId = await winCoinsContract.createEvent(
        name,
        outcomes,
        predictionDuration,
        oracleAddress
  ...
  renderEvents() {
    ...
    if (filter === "open") {
      ...
      filteredEvents = this.events.filter(
        (event) => event.isResolved && !event.isCancelled
      );
    ...
    return `
            <div class="event-card ${cardClass}" data-event-id="${event.id}">
                ...
                <div class="event-deadline">
                    ...
                    <span class="deadline-time">${winCoinsContract.formatTime(
                      event.predictionDeadline
                    )}</span>
                </div>
                <div class="event-footer">
                    ...
                    <span class="total-pool">${event.totalPoolAmount} ${
  ...
  async openEventModal(eventId) {
    ...
    const outcomesHtml = event.outcomes
      .map((outcome, index) => {
        ...
        return `
                <div class="outcome-card ${winnerClass} ${
          canPredict ? "clickable" : ""
        }" data-outcome-index="${index}">
                    <div class="outcome-header">
                        <div class="outcome-name">${outcome} ${
          isWinner ? "👑" : ""
        }</div>
                        <div class="outcome-pool">${poolAmounts[index]} ${
          this.currentNetworkSymbol
        }</div>
                    </div>
                    ${
            ...
            <div class="modal-header">
                ...
                <div class="event-meta">
                    ...
                    <span class="event-deadline-modal">
                        Deadline: ${winCoinsContract.formatTime(
                          event.predictionDeadline
                        )}
                    </span>
                </div>
  ...
  async submitPrediction(eventId) {
    const selectedOutcome = document.querySelector(
      'input[name="selectedOutcome"]:checked'
    );
  ...
  async makePrediction(eventId, outcomeIndex, amount) {
    ...
    try {
      ...
      const decimalAmount =
        amount instanceof Decimal ? amount : new Decimal(amount || "0");
      if (decimalAmount.lte(0)) {
        this.showNotification(
          "Please enter a valid prediction amount.",
          "error"
        );
        return;
      }
      ...
      this.showNotification(
        "Making prediction... Please confirm the transaction.",
        "info"
      );

      await winCoinsContract.makePrediction(
        eventId,
        outcomeIndex,
      ...
      if (selectedCard) {
        ...
        if (controls) {
          ...
          const amountInput = controls.querySelector(
  ...
  async loadUserPredictions() {
    ...
    try {
      this.userPredictions =
        await winCoinsContract.getUserPredictionsForAllEvents();
      this.renderUserPredictions();
    } catch (error) {
      console.error("Failed to load user predictions:", error);
      document.getElementById("userPredictionsContainer").innerHTML =
        "<p>Failed to load your predictions.</p>";
    }
  }
        ...
        return `
                <div class="${cardClasses}">
                    <div class="prediction-info">
                        ...
                        <p><strong>Prediction Amount:</strong> ${
                          prediction.predictionAmount
                        } ${
          prediction.predictionAmount !== "Claimed" &&
          prediction.predictionAmount !== "Refunded"
            ? "ETH"
            : ""
        }</p>
                        ${
                    ...
                    <div class="prediction-actions">
                        ${
                          canClaim
                            ? `<button class="btn btn-success btn-small" onclick="app.claimPayout(${prediction.eventId})">Claim Payout</button>`
                            : ""
                        }
                        ${
                          canClaimRefund
                            ? `<button class="btn btn-warning btn-small" onclick="app.claimPayout(${prediction.eventId})">Claim Refund</button>`
                            : ""
                        }
                    </div>
                </div>
  ...
  async checkRefundStatus(eventId) {
    try {
      ...
      const [hasClaimedRefund, claimedAmountBN] =
        await winCoinsContract.contract.getUserClaimInfo(
          this.userAddress,
          eventId
        );
      const claimedAmount = hasClaimedRefund
  ...
  async claimPayout(eventId) {
    ...
    try {
      ...
      this.showNotification(
        isRefund
          ? "Claiming refund... Please confirm the transaction."
          : "Claiming payout... Please confirm the transaction.",
        "info"
      );
  ...
  async loadAdminEvents() {
    if (!this.isConnected) {
      document.getElementById("adminEventsContainer").innerHTML =
        '<div class="loading">Please connect your wallet to manage events.</div>';
      return;
    }
    ...
    try {
      ...
      const adminEvents = this.events.filter(
        (event) =>
      ...
      document.getElementById("adminEventsContainer").innerHTML =
        '<div class="loading">Failed to load your events.</div>';
    }
  }
  ...
  async renderAdminEvents(adminEvents) {
    ...
    if (adminEvents.length === 0) {
      container.innerHTML =
        '<div class="loading">You haven\'t created any events yet.</div>';
      return;
    }

    // Get creator's current fee balance and total earned
    const creatorFeeBalance = await winCoinsContract.getCreatorFeeBalance();
    const totalCreatorFeesEarned =
      await winCoinsContract.getTotalCreatorFeesEarned();
                      ...
                      canResolve
                        ...
                        <div class="resolve-section">
                            ...
                            <div class="resolve-form">
                                ...
                                <button class="btn btn-success btn-small" onclick="app.resolveEvent(${
                                  event.id
                                })">
                                    Resolve Event
                                </button>
  ...
  async resolveEvent(eventId) {
    ...
    try {
      const winningOutcomeSelect = document.getElementById(
        `winningOutcome${eventId}`
      );
      const winningOutcome = parseInt(winningOutcomeSelect.value);

      this.showNotification(
        "Resolving event... Please confirm the transaction.",
        "info"
      );

      const receipt = await winCoinsContract.resolveEvent(
        eventId,
        winningOutcome
      );

      // Check for PlatformFeeCollected event to get creator fee info
      const feeCollectedEvent = receipt.events.find(
        (e) => e.event === "PlatformFeeCollected"
      );

      let creatorFeeMessage = "";
      if (feeCollectedEvent) {
        const creatorFeeAmount = ethers.utils.formatEther(
          feeCollectedEvent.args.creatorFeeAmount
        );
      ...
      this.showNotification(
        `Event resolved successfully!${creatorFeeMessage}`,
        "success"
      );

      // Show modal with detailed resolution results
      if (creatorFeeMessage) {
        this.showEventResolutionModal(
          eventId,
          winningOutcome,
          feeCollectedEvent
        );
  ...
  async cancelEvent(eventId) {
    ...
    try {
      this.showNotification(
        "Cancelling event... Please confirm the transaction.",
        "info"
      );
      ...
      this.showNotification(
        "Event cancelled successfully! Participants can now claim refunds.",
        "success"
      );
  ...
  async showEventResolutionModal(eventId, winningOutcome, feeCollectedEvent) {
    ...
    const platformFeeAmount = ethers.utils.formatEther(
      feeCollectedEvent.args.platformFeeAmount
    );
    const creatorFeeAmount = ethers.utils.formatEther(
      feeCollectedEvent.args.creatorFeeAmount
    );
    const totalFee = new Decimal(platformFeeAmount).add(
      new Decimal(creatorFeeAmount)
    );
                ...
                <div class="fee-breakdown">
                    ...
                    <div class="fee-details">
                        ...
                        <div class="fee-item total">
                            ...
                            <span class="fee-amount">${totalFee.toFixed(
                              6
                            )} ${currencySymbol}</span>
                        </div>
                    </div>
  ...
  async withdrawCreatorFees() {
    ...
    try {
      this.showNotification(
        "Withdrawing creator fees... Please confirm the transaction.",
        "info"
      );
      ...
      const withdrawEvent = receipt.events.find(
        (e) => e.event === "CreatorFeeWithdrawn"
      );
      const withdrawnAmount = withdrawEvent
        ? ethers.utils.formatEther(withdrawEvent.args.amount)
        : "0";
      const currencySymbol = await this.getCurrentCurrencySymbol();

      this.showNotification(
        `Creator fees withdrawn successfully! You received ${withdrawnAmount} ${currencySymbol}`,
        "success"
      );
      ...
      this.showNotification(
        "Failed to withdraw creator fees. Please try again.",
        "error"
      );
    }
  }
  ...
  async calculateAccurateOriginalPool(event) {
    ...
    try {
      ...
      const winningPoolAmountStr = await winCoinsContract.getPoolAmount(
        event.id,
        event.winningOutcome
      );

      // Convert to wei (BigNumber) for precise calculation
      const currentTotalPoolWei = ethers.utils.parseEther(
        event.totalPoolAmount
      );
